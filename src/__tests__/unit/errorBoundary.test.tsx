...
describe('ErrorBoundary', () => {
  describe('Fallback Order', () => {
    ...
    it('should show "Try Again" button as primary action', () => {
      ...
      expect(tryAgainButton).toHaveClass('bg-primary'); // Primary styling
    });
    ...
    it('should show "Refresh Page" as secondary action', () => {
      ...
      expect(refreshButton).toHaveClass('variant-outline'); // Secondary styling
    });
  ...
  describe('No Reload Behavior', () => {
    ...
    it('should use window.location.reload only for "Refresh Page"', async () => {
      const user = userEvent.setup();
      const reloadSpy = vi.spyOn(window.location, 'reload').mockImplementation(() => {});
...
describe('NavigationErrorBoundary', () => {
  describe('Navigation-Specific Fallbacks', () => {
    ...
    it('should prioritize back navigation as primary action', () => {
      render(
        <NavigationErrorBoundary>
          <ThrowError shouldThrow={true} />
        </NavigationErrorBoundary>,
      ...
      expect(backButton).toHaveClass('bg-primary');
    });

    it('should provide "Try Again" as secondary action', () => {
      render(
        <NavigationErrorBoundary>
          <ThrowError shouldThrow={true} />
        </NavigationErrorBoundary>,
      ...
      const tryAgainButton = screen.getByRole('button', { name: /try again/i });
      expect(tryAgainButton).toBeInTheDocument();
      expect(tryAgainButton).toHaveClass('variant-outline');
    });

    it('should not reload on back navigation', async () => {
      const user = userEvent.setup();
      const reloadSpy = vi.spyOn(window.location, 'reload').mockImplementation(() => {});
